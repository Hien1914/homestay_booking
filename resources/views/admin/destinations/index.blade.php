...
@section('content')
    <div class="admin-page-header">
        ...
        <div class="admin-page-actions">
            <a href="{{ route('admin.destinations.create') }}" class="admin-create-btn">
    ...
    <div class="card border-0 shadow-sm mb-4 rounded-3">
        <div class="card-body p-4">
            <form method="GET" action="{{ route('admin.destinations') }}" class="row g-3 align-items-end">
                <div class="col-md-5">
    ...
    <div class="card border-0 shadow-sm mb-4 rounded-3">
        ...
        <div class="card-body p-0">
                    ...
                    <tbody>
                        @forelse($pendingDestinations as $destination)
                            <tr>
                                ...
                                <td>
                                    <span class="admin-badge admin-badge-info">{{ $destination->homestays_count ?? 0 }}</span>
                                </td>
                                <td>{{ optional($destination->created_at)->format('d/m/Y') }}</td>
                                <td>
                                    <div class="d-flex gap-1 justify-content-center">
                                        <a href="{{ route('admin.destinations.edit', $destination) }}" class="admin-action-btn"
                                            title="Chỉnh sửa">
                                            <i class="bi bi-pencil"></i>
                                        </a>
                                        <button type="button" class="admin-action-btn" data-bs-toggle="modal"
                                            data-bs-target="#destModal{{ $destination->id }}" title="Xem chi tiết">
                                            <i class="bi bi-eye"></i>
                                        </button>
            ...
            <div class="mt-4 d-flex justify-content-center">
                {{ $pendingDestinations->appends(request()->except('pending_page'))->links() }}
            </div>
        </div>
    </div>
        ...
        <div class="card-body p-0">
            <div class="admin-table-wrap">
                <table class="admin-table">
                    ...
                    <tbody>
                        @forelse($approvedDestinations as $destination)
                            <tr>
                                <td>
                                    @if($destination->image)
                                        <img src="{{ asset('storage/' . $destination->image) }}" alt="{{ $destination->name }}"
            ...
            <!-- Phân trang -->
            <div class="mt-4 d-flex justify-content-center">
                {{ $approvedDestinations->appends(request()->except('approved_page'))->links() }}
            </div>
        </div>
    </div>

    @foreach($pendingDestinations as $destination)
        <div class="modal fade" id="destModal{{ $destination->id }}" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Chi tiết đơn duyệt: {{ $destination->name }}</h5>
                    ...
                    <div class="modal-body">
                        <div class="text-center mb-3">
                            @if($destination->image)
                                <img src="{{ asset('storage/' . $destination->image) }}" class="img-fluid rounded"
                                    alt="{{ $destination->name }}" style="max-height: 200px;">
                            @else
                        ...
                        <p class="text-muted" style="white-space: pre-wrap;">{{ $destination->description ?: 'Không có mô tả' }}
                        </p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Đóng</button>
                        <form action="{{ route('admin.destinations.approve', $destination) }}" method="POST" class="d-inline">
                            @csrf
                            @method('PUT')
