...
@section('title', isset($homestay) ? 'Chỉnh sửa chỗ nghỉ' : 'Thêm chỗ nghỉ mới')
...
<div class="admin-page-header">
    <div class="admin-page-header-content">
        ...
        <p class="admin-page-subtitle">{{ $isEdit ? 'Cập nhật thông tin chỗ nghỉ' : 'Điền thông tin để tạo chỗ nghỉ mới. Sau khi gửi, chỗ nghỉ sẽ chờ admin duyệt.' }}</p>
    </div>
</div>
...
<div class="p-2">
    <form action="{{ $formAction }}" method="POST" enctype="multipart/form-data" id="homestay-form" class="homestay-form">
            ...
            <div class="card-body p-4">
                <div class="row g-4">
                    <div class="col-12">
                        ...
                        <input type="text" id="title" name="title" value="{{ old('title', $homestay->title ?? '') }}" class="form-control rounded-3" placeholder="VD: Villa Đà Lạt view đồi thông" required>
                    </div>

                    <div class="col-12">
                        <label for="slug" class="form-label fw-bold">Đường dẫn (Slug)</label>
                        <input type="text" id="slug" name="slug" value="{{ old('slug', $homestay->slug ?? '') }}" class="form-control bg-light rounded-3" placeholder="Tự động tạo từ tiêu đề" readonly>
                    </div>

                    <div class="col-12">
                        <label for="description" class="form-label fw-bold">Mô tả chi tiết <span class="text-danger">*</span></label>
                        <textarea id="description" name="description" class="form-control rounded-3" rows="8" placeholder="Mô tả chi tiết về homestay (tối thiểu 100 ký tự)" required minlength="100">{{ old('description', $homestay->description ?? '') }}</textarea>
                    ...
                    <div class="card-body p-4">
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                ...
                                <input type="text" id="destination_custom" name="destination_custom" value="{{ old('destination_custom', '') }}" class="form-control rounded-3" placeholder="VD: Đà Lạt, Hội An...">
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <input type="text" id="address" name="address" value="{{ old('address', $homestay->address ?? '') }}" class="form-control rounded-3" placeholder="Số nhà, tên đường, khu vực..." required>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body p-4">
                        <div class="row g-4">
                            <div class="col-12">
                                ...
                                <input type="number" id="price_per_night" name="price_per_night" value="{{ old('price_per_night', $homestay->price_per_night ?? '') }}" class="form-control rounded-3" min="0" step="1000" placeholder="500.000" required>
                            ...
                            <div class="col-12">
                                ...
                                <input type="number" id="max_guests" name="max_guests" value="{{ old('max_guests', $homestay->max_guests ?? 2) }}" class="form-control rounded-3" min="1" max="50" required>
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <select id="promotion_id" name="promotion_id" class="form-select rounded-3">
                                    ...
                                    @foreach($promotions as $promo)
                                        <option value="{{ $promo->id }}" @selected(old('promotion_id', $homestay->promotion_id ?? '') == $promo->id)>
                    ...
                    @foreach($roomTypes as $type => $label)
                        ...
                        <div class="col-6 col-md-3">
                            <div class="p-3 bg-light rounded-3 border h-100">
                                <label for="room_{{ $type }}" class="form-label fw-bold small text-secondary mb-2">{{ $label }}</label>
                                <input type="number" id="room_{{ $type }}" name="rooms[{{ $type }}]" value="{{ old("rooms.{$type}", $quantity) }}" class="form-control rounded-2" min="0" max="100" placeholder="0">
                            </div>
                        </div>
                    @endforeach
                </div>
                            ...
                            <div class="p-3 bg-light rounded-3 border h-100">
                                <label class="d-flex align-items-center gap-2 mb-2 cursor-pointer">
                                    <input type="checkbox" name="amenities[]" value="{{ $amenity->id }}" 
                                        class="form-check-input amenity-checkbox-input" @checked($isChecked)>
                                    <span class="fw-bold small text-secondary">{{ $amenity->name }}</span>
                                </label>
                                <input type="number" name="amenity_quantities[{{ $amenity->id }}]" 
                                    class="form-control rounded-2 bg-white" value="{{ $oldQuantity }}" min="1" max="100" 
                                    placeholder="SL: 1" @disabled(!$isChecked)>
                            </div>
                        </div>
                    @endforeach
                </div>
                            ...
                            <div class="d-flex flex-wrap gap-3">
                                @foreach($roomImages as $image)
                                    <div class="admin-image-item position-relative rounded-3 overflow-hidden shadow-sm border">
                                        <img src="{{ asset('storage/' . $image->image_url) }}" alt="Ảnh phòng" class="object-fit-contain" style="height: 120px; width: auto; max-width: 200px; display: block;">
                                        <button type="button" class="admin-image-delete btn btn-danger btn-sm rounded-circle shadow position-absolute" style="top:5px; right:5px;" onclick="deleteImage(this, {{ $image->id }})">
                                            <i class="bi bi-x"></i>
                                        </button>
                                        <input type="hidden" name="delete_images[]" value="{{ $image->id }}" class="d-none" disabled>
                                    </div>
                                @endforeach
                            </div>
                        </div>
        ...
        <div class="admin-form-actions d-flex justify-content-end gap-2 mb-5">
            <a href="{{ route('host.homestays.index') }}" class="admin-filter-clear-btn px-4 d-flex align-items-center">
                Hủy bỏ
            </a>
            <button type="submit" class="admin-create-btn px-5">
                <i class="bi bi-check-circle me-2"></i>{{ $submitLabel }} chỗ nghỉ
            </button>
        </div>
    </form>
</div>
