  ...
  <section class="homestay-detail-page">
    <div class="container-setting">
      ...
      <div class="homestay-detail-success" id="booking-success-msg" role="status" hidden>
        ...
        <span>Đặt phòng thành công! Mã đặt phòng: <strong id="booking-code-display"></strong>. Vui lòng tiến hành thanh
          toán.</span>
      </div>
      ...
      <header class="homestay-detail-hero">
        ...
        <div class="homestay-detail-hero__stars">
          @for($i = 1; $i <= 5; $i++)
            <i
              class="fa-solid fa-star homestay-detail-hero__star {{ $i <= round($homestay->reviews_avg_rating ?? 0) ? 'is-filled' : '' }}"></i>
          @endfor
          <span class="ms-2 text-muted"
            style="font-size: 0.9rem;">({{ number_format((float) ($homestay->reviews_avg_rating ?? 0), 1, ',', '.') }})</span>
        </div>
      ...
      <div class="homestay-detail-gallery-slider">
        @if($imgs->count() > 0)
          <div class="homestay-detail-gallery-track" id="homestay-gallery-track">
            @foreach($imgs as $index => $image)
              <div class="homestay-detail-gallery-slide">
                <img src="{{ asset('storage/' . $image->image_url) }}" alt="{{ $homestay->title }} - ảnh {{ $index + 1 }}"
                  loading="{{ $index === 0 ? 'eager' : 'lazy' }}" data-gallery-image data-gallery-index="{{ $index }}">
              </div>
            @endforeach
          </div>
          @if(count($imgs) > 1)
            <button type="button" class="homestay-detail-gallery-nav homestay-detail-gallery-nav--prev" id="gallery-prev"
              aria-label="Ảnh trước">
              <i class="fa-solid fa-chevron-left"></i>
            </button>
            <button type="button" class="homestay-detail-gallery-nav homestay-detail-gallery-nav--next" id="gallery-next"
              aria-label="Ảnh sau">
          ...
          <div class="homestay-detail-gallery-thumbs" id="homestay-gallery-thumbs">
            @foreach($imgs as $index => $image)
              <button type="button" class="homestay-detail-gallery-thumb {{ $index === 0 ? 'is-active' : '' }}"
      ...
      <div class="homestay-detail-lightbox" id="homestay-lightbox" hidden>
        ...
        @if(count($imgs) > 1)
          <button type="button" class="homestay-detail-lightbox-nav homestay-detail-lightbox-nav--prev" id="lightbox-prev"
            aria-label="Ảnh trước">
            <i class="fa-solid fa-chevron-left"></i>
          </button>
          <button type="button" class="homestay-detail-lightbox-nav homestay-detail-lightbox-nav--next" id="lightbox-next"
            aria-label="Ảnh sau">
            <i class="fa-solid fa-chevron-right"></i>
          </button>
        @endif
      ...
      <div class="homestay-detail-layout">
        <div class="homestay-detail-main">
          ...
          <div class="homestay-detail-section homestay-detail-card" id="tien-nghi">
            ...
            <div class="homestay-detail-rooms-grid mb-4">
              <h3 class="homestay-detail-sub-title mb-3" style="font-size: 1.1rem; color: #333;"><i
                  class="bi bi-door-open me-2"></i>Các phòng hiện có</h3>
                  ...
                  @if($room->quantity > 0)
                    <div class="col">
                      <div class="p-2 border rounded bg-light d-flex align-items-center">
                        ...
                        <span>{{ $room->quantity }}
                          {{ \App\Models\HomestayRoom::ROOM_TYPES[$room->feature_type] ?? $room->feature_type }}</span>
                      </div>
                    </div>
                  @endif
                @endforeach
            ...
            <div class="homestay-detail-amenities-list">
              <h3 class="homestay-detail-sub-title mb-3" style="font-size: 1.1rem; color: #333;"><i
                  class="bi bi-stars me-2"></i>Tiện nghi chỗ nghỉ</h3>
              <div class="row row-cols-2 row-cols-md-3 g-3">
                @foreach($homestay->amenities as $amenity)
                  <div class="col">
                    <div class="d-flex align-items-center">
                      <i class="bi bi-check2 text-success me-2" style="font-size: 1.2rem;"></i>
                      <span>{{ $amenity->name }} @if($amenity->pivot->quantity > 1) (x{{ $amenity->pivot->quantity }})
                      @endif</span>
                    </div>
                  </div>
                @endforeach
          ...
          <div class="homestay-detail-section homestay-detail-card" id="danh-gia">
            ...
            @auth
              @if($reviewableBooking)
                <form method="POST"
                  action="{{ route('homestay.review.store', ['slug' => $homestay->slug, 'booking' => $reviewableBooking->id]) }}"
                  class="homestay-detail-review-form">
                  @csrf
                  ...
                  <textarea name="comment" rows="3" maxlength="1000" placeholder="Chia sẻ trải nghiệm thực tế của bạn..."
                    required>{{ old('comment') }}</textarea>
                  <button type="submit" class="btn btn-primary">Gửi đánh giá</button>
                </form>
              @elseif(auth()->user())
                <p class="homestay-detail-empty-reviews">Bạn chỉ có thể đánh giá sau khi đã hoàn thành một booking tại
                  homestay này.</p>
              @endif
            @else
              <p class="homestay-detail-empty-reviews">Vui lòng <a
                  href="{{ route('login.page') }}?redirect={{ urlencode(request()->fullUrl()) }}">đăng nhập</a> để gửi đánh
                giá.</p>
            @endauth
            ...
            @else
              <p class="homestay-detail-empty-reviews">Chưa có đánh giá nào. Hãy là người đầu tiên trải nghiệm và đánh giá
                homestay này.</p>
            @endif
          </div>
        </div>
        ...
        <aside class="homestay-detail-sidebar" id="dat-phong">
          ...
          <div class="homestay-detail-sidebar-card">
            <p class="homestay-detail-sidebar-text"><strong>Nhận phòng:</strong>
              {{ $homestay['check_in_time'] ?? '14:00' }}</p>
            <p class="homestay-detail-sidebar-text"><strong>Trả phòng:</strong>
              {{ $homestay['check_out_time'] ?? '12:00' }}</p>
          </div>
          ...
          <div class="homestay-detail-card homestay-detail-card--booking">
            <form id="homestay-booking-form" class="homestay-booking-form">
              <div class="homestay-booking-price">
                ...
                @if($hasDiscount)
                  <span
                    style="margin-left:8px;font-size:.86rem;color:#9ca3af;text-decoration:line-through;">{{ number_format($originalPricePerNight) }}đ</span>
                @endif
              </div>
              ...
              <div class="homestay-booking-fields">
                ...
                <div class="homestay-booking-field">
                  ...
                  <div class="homestay-booking-stepper">
                    <button type="button" class="homestay-booking-stepper-btn" data-delta="-1"
                      aria-label="Giảm">−</button>
                    <input type="number" id="num_guests" name="num_guests" value="1" min="1" max="{{ $maxGuests + 10 }}"
                      readonly>
                    <button type="button" class="homestay-booking-stepper-btn" data-delta="1" aria-label="Tăng">+</button>
                  </div>
                  <span class="homestay-booking-stepper-hint">Tiêu chuẩn {{ $maxGuests }} khách. Vượt quá:
                    +{{ number_format($extraGuestFee, 0, ',', '.') }}đ/người/đêm</span>
                </div>
              ...
              <div class="homestay-detail-login-notice" id="homestay-login-notice"
                style="{{ $isLoggedIn ? 'display: none;' : '' }}">
                <i class="fa-solid fa-circle-exclamation"></i>
                <span>Vui lòng <a href="{{ route('login.page') }}?redirect={{ urlencode(request()->fullUrl()) }}">Đăng
                    nhập</a> để đặt phòng.</span>
              </div>
              <div class="homestay-booking-error" id="homestay-booking-error" role="alert" hidden></div>

              <button type="submit" class="btn btn-primary homestay-detail-book-btn" id="homestay-booking-submit"
                @if(!$isDemo && !$isLoggedIn) style="display: none;" @endif>
                Thanh toán
              </button>
            </form>
          </div>
    ...
    <section class="py-5">
      <div class="container-setting">
        <div class="d-flex justify-content-between align-items-center mb-4">
          ...
          <a href="{{ route('pages.search') }}" class="text-decoration-none fw-semibold"
            style="color: var(--green-900);">Xem thêm</a>
        </div>
        <div class="row g-4">
          @foreach($related as $item)
            ...
            <div class="col-12 col-md-6 col-lg-4">
              <a href="{{ route('homestay.show', ['slug' => $item->slug]) }}"
                class="text-decoration-none text-dark d-block h-100">
                <article class="card h-100 border-0 shadow-sm rounded-4 overflow-hidden">
                  <div style="height: 200px; overflow: hidden;">
                    @if($coverImage)
                      <img src="{{ asset('storage/' . $coverImage) }}" alt="{{ $item->title }}" class="w-100 h-100"
                  ...
                  <div class="card-body d-flex flex-column gap-3 p-4">
                    ...
                    <div class="d-flex align-items-center justify-content-between mt-auto">
                      <div class="small text-warning fw-semibold">
                        <i
                          class="fa-solid fa-star me-1"></i>{{ number_format((float) ($item->reviews_avg_rating ?? 0), 1, ',', '.') }}
